 ...
 * @param {Array} files
 * @param {Object} [goalOverrides] per-section custom hour targets, keyed by section id
 */
function assessVaultStats(files, goalOverrides) {
  const overrides = goalOverrides && typeof goalOverrides === 'object' ? goalOverrides : {};
  ...
  const sections = HOUR_SECTIONS.map((sec) => {
    ...
    const custom = Number(overrides[sec.id]);
    const target = Number.isFinite(custom) && custom > 0 ? custom : sec.targetHours;
    ...
    return {
      ...
      customTarget: target !== sec.targetHours,
      pct,
      color: sec.color,
    };
  });
